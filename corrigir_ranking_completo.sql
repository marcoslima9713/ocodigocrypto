-- Script completo para corrigir o ranking
...
-- 3. Configurar RLS (Row Level Security) para a tabela
...
-- 6. Configurar privacidade para aparecer no feed
...
-- 7. Limpar transações existentes do usuário
DELETE FROM public.transactions WHERE user_id = '856d169f-8563-4126-a348-fdedb4f3259f';

-- 8. Inserir transações de teste (DCA - Dollar Cost Averaging)
...
-- 9. Criar função para calcular rankings (com requisito de 1 hora em vez de 7 dias)
...
BEGIN
    ...
    WHERE 
        u.created_at < NOW() - INTERVAL '1 hour'  -- Mudança de 7 dias para 1 hora
...
-- 10. Executar o cálculo dos rankings
SELECT public.calculate_portfolio_rankings();

-- 11. Verificar se o usuário Marcos aparece no ranking
SELECT 
    user_id,
    user_name,
    user_username,
...
-- 12. Mostrar todos os rankings ordenados por retorno
SELECT 
    user_name,
    user_username,
    time_window,
